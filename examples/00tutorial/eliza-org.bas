...
25 REM "(Almost) original code to test HASMSSTRINGS"
30 DIM C1$(80)
40 GOSUB 1070
...
223 IF (I$(L))>=97 AND (I$(L))<=122 THEN T$(L)=CHR$((I$(L))-32): ELSE T$(L)=I$(L)
224 NEXT
225 I$=T$
230 T$=" "+I$+" ": I$=T$
...
270 T$=MID$(I$, 1, L-1)+MID$(I$, L+1, LEN(I$)-L)
275 I$=T$
280 GOTO 260
290 IF L+4>=LEN(I$) THEN 330
300 IF MID$(I$, L, 4) <> "SHUT" THEN 330
...
750 T$=MID$(C$,1,L-1)+R$+MID$(C$,L+LEN(S$),LEN(C$)-(L+LEN(S$)-1))
755 C$=T$
760 GOTO 810
770 IF L+LEN(R$)>LEN(C$) THEN 810
780 IF MID$(C$, L, LEN(R$)) <> R$ THEN 810
790 T$=MID$(C$,1,L-1)+S$+MID$(C$,L+LEN(R$),LEN(C$)-(L+LEN(R$)-1))
795 C$=T$
800 L=L+LEN(S$)
810 NEXT L
820 NEXT X
