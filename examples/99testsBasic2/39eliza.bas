10 REM "A ELIZA version from the 70s"
20 REM "Ported 2022 - Stefan Lenz"
30 DIM C1$(80)
40 GOSUB 1070
...
210 REM
215 RESTORE 243+ZZ: ZZ=ZZ+1
220 READ I$: PRINT I$
221 REM "Uppercase the input"
222 FOR L=1 TO LEN(I$)
223 IF I$(L)>=97 AND I$(L)<=122 THEN T$(L)=(I$(L))-32 ELSE T$(L)=I$(L)
224 NEXT
225 I$=T$
230 REM I$ = " " + I$ + " " 
235 T$=" " : T$(2)=I$: T$(LEN(T$)+1)=" "
236 I$=T$
240 REM  GET RID OF APOSTROPHES
250 FOR L = 1 TO LEN(I$)
260 REM IF MID$(I$, L, 1) <> "'" THEN 290
261 IF I$(L, L+1)<>"'" THEN 290
270 REM I$ = MID$(I$, 1, L - 1) + MID$(I$, L + 1, LEN(I$) - L)
271 T$=I$(1, L-1): T$(L)=I$(L+1)
280 GOTO 260
290 IF L+4>LEN(I$) THEN 330
300 REM IF UCASE$(MID$(I$, L, 4)) <> "SHUT" THEN 330
301 IF I$(L, L+3)<>"SHUT" THEN 330
...
460 REM IF MID$(I$, L, LEN(K$)) <> K$ THEN 500
461 IF I$(L, L+LEN(K$)-1)<>K$ THEN 500
470 S=K
480 T=L
490 F$=K$
500 NEXT L
...
670 L=L-1
680 GOTO 660
690 REM C$ = " " + MID$(I$, LEN(F$) + L, LEN(I$) - (LEN(F$) + L - 1))
691 T=LEN(I$)-(LEN(F$)+L-1)
692 U=LEN(F$)+L
693 C$=I$(U, U+T-1)
700 FOR X=1 TO N2/2
710 READ S$
...
740 REM IF MID$(C$, L, LEN(S$)) <> S$ THEN 770
741 IF C$(L, L+LEN(S$)-1)<>S$ THEN 770
750 REM C$=MID$(C$,1,L-1)+R$+MID$(C$,L+LEN(S$),LEN(C$)-(L+LEN(S$)-1))
751 T$=C$(1, 1+(L-1)-1)
752 T$(LEN(T$)+1)=R$
753 T=L+LEN(S$)
754 U=LEN(C$)-(L+LEN(S$)-1)
755 T$(LEN(T$)+1)=C$(T, T+U-1)
756 C$=T$
760 GOTO 810
770 IF L+LEN(R$)>LEN(C$) THEN 810
780 REM IF MID$(C$, L, LEN(R$)) <> R$ THEN 810
781 IF C$(L, L+LEN(R$)-1)<>R$ THEN 810
790 REM C$=MID$(C$,1,L-1)+S$+MID$(C$,L+LEN(R$),LEN(C$)-(L+LEN(R$)-1))
791 T$=C$(1, 1+(L-1)-1)
792 T$(LEN(T$)+1)=S$
793 T=L+LEN(R$)
794 U=LEN(C$)-(L+LEN(R$)-1)
795 T$(LEN(T$)+1)=C$(T, T+U-1)
796 C$=T$
800 L=L+LEN(S$)
810 NEXT L
820 NEXT X
830 REM IF MID$(C$, 2, 1)=" " THEN C$=MID$(C$,2,LEN(C$)-1) 'ONLY 1SPACE
831 IF C$(2,2)=" " THEN C$=C$(2, 2+LEN(C$)-1-1)
840 REM
...
960 REM IF MID$(F$, LEN(F$), 1) = "*" THEN 1000
961 T=LEN(F$)
962 IF F$(T, T)="*" THEN 1000
970 PRINT F$
980 P$=I$
990 GOTO 190
1000 REM PRINT MID$(F$, 1, LEN(F$) - 1); C$
1001 PRINT F$(1, 1+(LEN(F$)-1)-1); C$
1010 P$=I$
1020 GOTO 190
1030 REM  "PRINT CENTER ROUTINE"
1040 TAB(40-LEN(C1$)/2): PRINT C1$
1060 RETURN
1070 C1$ = "*** ELIZA ***"
1080 GOSUB 1030
1090 C1$ = "IN BASIC"
...
2580  DATA  "MY FAVORITE CAR IS A LAMBORGHINI COUNTACH. WHAT IS YOUR'S?"
...
2800  REM  "'ELIZA'"
2810  END
3000  REM "Test dialog questions"
3010  DATA "I am bored!", "I love cars too much."
3020  DATA "Yes I like cars very much.", "Totally" 
3030  DATA "This is so annoying", "You are testing me"
3040  DATA "Or is it me testing you?", "Shut up"
