...
1600 FORI=1TO3:K(I,3)=0:NEXTI:REM "Q$=Z$+Z$+Z$+Z$+Z$+Z$+Z$+LEFT$(Z$,17)"
1610 FOR I=1 TO 6:Q$(LEN(Q$)+1)=Z$: NEXT: Q$(LEN(Q$)+1)=Z$(1,17)
...
2080 FORI=1TO9:IFA$(1,3)<>A1$(3*I-2,3*I)THEN2160
...
3240 S8=INT(S1)*24+INT(S2)*3-26:IFQ$(S8,S8+1)="  "THEN3360
...
5150 FOR I=1TO3:IF X3=K(I,1)ANDY3=K(I,2)THEN5190
5180 NEXTI:I=3
...
5580 IF X<0 OR S=X THEN PRINT"<SHIELDS UNCHANGED>":GOTO1990
...
5920 GOSUB8790:PRINT G2$;: TAB 25: PRINT INT(D(R1)*100)*.01
5950 NEXT R1:PRINT:IF D0<>0 THEN5720
5980 GOTO 1990
5990 REM "KLINGONS SHOOTING"
6000 IF K3<=0THEN RETURN
6010 IF D0<>0THEN PRINT"STARBASE SHIELDS PROTECT THE ENTERPRISE":RETURN
6040 FOR I=1TO3:IFK(I,3)<=0THEN6200
...
6120 IF RND(1)>.6ORH/S<=.02THEN6200
...
6820 FORJ=(I-1)*24+1TO(I-1)*24+22STEP3:PRINT" ";Q$(J,J+2);:NEXTJ
...
7720 NEXTJ:GOTO7850
7740 Z4=I:Z5=1:GOSUB9030:J0=INT(15-.5*LEN(G2$)):TAB J0:PRINT G2$;
...
8680 IFS8=1THEN TT$=A$: TT$(LEN(TT$)+1)=Q$(Q1,Q2):Q$=TT$: RETURN
8690 IFS8=190THENQ$=Q$(1,189):Q$(190)=A$:RETURN
...
8890 IF Q$(S8,S8+2)<>A$THENRETURN
8900 Z3=1:RETURN
...
9200 G2$="SPICA"
9210 IFG5<>1THEN ON Z5 GOTO 9230,9240,9250,9260,9230,9240,9250,9260
9220 RETURN
